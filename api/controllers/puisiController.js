...
class PuisiController {
    // Get all puisi dengan pagination dan filter
    static async getAllPuisi(req, res) {
        try {
    ...
    static async getPuisiById(req, res) {
        try {
    ...
    // Create new puisi
    static async createPuisi(req, res) {
        try {
            ...
            console.error('Create puisi error:', error);
            res.status(500).json({ error: 'Terjadi kesalahan server' });
        }
    }

    // Update puisi
    static async updatePuisi(req, res) {
        try {
            const { judul, konten, kategori, musik } = req.body;
            const puisi = await Puisi.findById(req.params.id);
    ...
    // Delete puisi
    static async deletePuisi(req, res) {
        try {
            const puisi = await Puisi.findById(req.params.id);
    ...
    // Like/unlike puisi
    static async likePuisi(req, res) {
        try {
            const puisi = await Puisi.findById(req.params.id);
    ...
    // Get user's puisi
    static async getUserPuisi(req, res) {
        try {
            const page = parseInt(req.query.page) || 1;
            const limit = parseInt(req.query.limit) || 10;
    ...
    // Get popular puisi
    static async getPopularPuisi(req, res) {
        try {
...
module.exports = PuisiController;
